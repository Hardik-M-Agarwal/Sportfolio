  ...
  return (
    <div className="min-h-screen bg-[#f7f6f2]">
      ...
      <div className="max-w-6xl mx-auto px-6 py-10">
        {/* Page header */}
        <div className="mb-8">
          <h1 className="text-4xl font-black tracking-tight text-gray-900" style={{ fontFamily: "'Syne', sans-serif" }}>
            Browse Tournaments
          </h1>
          <p className="text-gray-500 mt-2">Find tournaments happening near you — register, sponsor, or just watch live.</p>
        </div>
        ...
        {loading ? (
          ...
          <>
            <p className="text-xs text-gray-400 mb-4 font-medium">{tournaments.length} tournament{tournaments.length !== 1 ? 's' : ''} found</p>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
              {tournaments.map((t) => (
                <TournamentCard
                  key={t._id}
                  tournament={t}
                  onClick={() => navigate(`/t/${t.tournamentCode}`)}
                />
              ))}
            </div>
          </>
...
function TournamentCard({ tournament: t, onClick }) {
  const prizePool = Math.round(t.entryFee * t.maxTeams * (t.prizeStructure?.percentage || 0) / 100);

  return (
    <div
      onClick={onClick}
      className="bg-white border border-gray-200 rounded-xl p-6 hover:shadow-md hover:-translate-y-0.5 transition-all cursor-pointer"
    >
      ...
      <div className="flex flex-col gap-2 mb-4">
        ...
        <div className="flex items-center gap-2 text-xs text-gray-500">
          ...
          {t.registeredCount ?? 0} / {t.maxTeams} teams
        </div>
      </div>
      ...
      <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden mb-4">
        <div
          ...
          style={{ width: `${Math.min(((t.registeredCount ?? 0) / t.maxTeams) * 100, 100)}%` }}
        />
      </div>
      ...
      <div className="mt-4 text-center text-xs font-semibold text-blue-600">
        View Tournament →
      </div>
    </div>
  );
